<template>
    <div class="container mx-auto flex flex-col gap-3 w-full px-5 py-3 rounded-md">
        <h1 class="text-left text-xl text-bold">{{weatherInfo.city}}, {{weatherInfo.country}}</h1>
        <div class="flex items-center justify-start gap-3 text-3xl text-bold">
            <img :src="iconURL" alt="" class="w-1/3"> <span>{{weatherInfo.temp}}&#8451</span>
        </div>
        <p>Feels like {{weatherInfo.feels_like}}&#8451. <span class="capitalize">{{weatherInfo.weather.description}}</span></p>
        <div class="flex justify-start gap-3">
            <span class="flex justify-start items-center gap-1">
                <span class="w-3" :style="rotation">
                    <svg version="1.1"
                        viewBox="0 0 488.98 488.98">
                        <g>
                            <g>
            ...
            {{weatherInfo.wind.speed.toFixed(1)}} m/s {{windDir}}</span> 
            <span class="flex justify-between items-center">
                <span >
                    <svg version="1.1" class="w-4"
                    viewBox="0 0 470 470">
                        <g>
        ...
        <div>
            <span>Visibility: {{(weatherInfo.visibility / 1000).toFixed(1)}}km</span>
        </div>
...
const iconURL: string = `http://openweathermap.org/img/wn/${props.weatherInfo.weather.icon}@2x.png`
const directions = ['N','NNW','NW','WNW','W','WSW','SW','SSW','S','SSE','SE','ESE','E','ENE','NE','NNE'];
const degList = ['180', '135', '135', '135', '90', '45', '45', '45', '0', '-45', '-45', '-45', '-90', '-135', '-135', '-135' ];
const index = Math.round(((props.weatherInfo.wind.deg %= 360) < 0 ? props.weatherInfo.wind.deg + 360 : props.weatherInfo.wind.deg) / 22.5) % 16;
const deg = degList[index];
