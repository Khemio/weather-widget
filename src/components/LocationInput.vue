<template>
    <div class="flex items-center justify-center relative">
        <input type="text" 
        v-model="locationInput"
        class="border border-black rounded-lg text-lg p-2"
        placeholder="Please Enter Location"
        @keyup.enter="handleInput">
        <span class="bg-black absolute right-0 top-0 h-full aspect-square flex items-center justify-center rounded-r-md cursor-pointer"
        @click="handleInput">
            <svg width="24px" height="24px" viewBox="0 0 24 24" version="1.1" >
                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g fill="#ffffff" fill-rule="nonzero">
                        <path d="M21.25,4 C21.6642136,4 22,4.33578644 22,4.75 L22,4.75 L22,11.25 C22,13.3210678 20.3210678,15 18.25,15 L18.25,15 L4.58583574,15 L8.30516583,18.7196699 C8.57143239,18.9859365 8.59563844,19.4026002 8.37778398,19.6962117 L8.30516583,19.7803301 C8.03889927,20.0465966 7.62223558,20.0708027 7.32862409,19.8529482 L7.24450566,19.7803301 L2.24450566,14.7803301 C1.97823909,14.5140635 1.95403304,14.0973998 2.1718875,13.8037883 L2.24450566,13.7196699 L7.24450566,8.71966991 C7.53739888,8.4267767 8.01227261,8.4267767 8.30516583,8.71966991 C8.57143239,8.98593648 8.59563844,9.40260016 8.37778398,9.69621165 L8.30516583,9.78033009 L4.58583574,13.5 L18.25,13.5 C19.440864,13.5 20.4156449,12.5748384 20.4948092,11.4040488 L20.5,11.25 L20.5,4.75 C20.5,4.33578644 20.8357864,4 21.25,4 Z" id="🎨-Color"></path>
                    </g>
                </g>
            </svg>
        </span>
    </div>
</template>

<script setup lang="ts">
import { ref } from "vue";
import { fetchGeoData } from '@/composables/store.js'

const locationInput = ref<String>('');

const handleInput = () => {
    fetchGeoData(locationInput.value);
    locationInput.value = '';

}


</script>


<style>

</style>